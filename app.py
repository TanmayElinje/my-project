...
st.set_page_config(
    page_title="AlphaVue",
    page_icon="🏦",  # Path to your favicon file
    layout="wide"
)
...
/* Simple fix: Replace the keyboard_double_arrow_right text with an actual arrow */
...
[data-testid="stIconMaterial"]:after {
    content: "⮞" !important; /* Unicode double arrow right */
    font-size: 20px !important;
    color: rgba(47, 79, 79, 0.8) !important;
...
/* Alternative arrows you can use instead of ⮞ */
        ...
        if not users_df.empty:
            ...
            with col1:
                with st.expander("✏️ Change User Role"):
                    user_to_change = st.selectbox("Select User by ID", users_df['user_id'].tolist(), key="selectbox_role_user")
                    new_role = st.selectbox("Select New Role", ['user', 'admin'], key="selectbox_new_role")
            ...
            with col2:
                with st.expander("🗑️ Delete User"):
                    ...
                    if not deletable_users.empty:
                        user_to_delete = st.selectbox("Select User by ID", deletable_users['user_id'].tolist(), key="selectbox_delete_user")
